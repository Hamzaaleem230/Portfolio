...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-background/80 backdrop-blur-md text-textMain z-50 shadow-md">
      ...
      {/* Mobile Sidebar */}
      <div
        className={`md:hidden fixed top-0 left-0 h-screen w-[75%] max-w-[320px] bg-gray-900 shadow-lg transform transition-transform duration-300 z-50 
          ...
          {["home", "about", "projects", "skills", "contact"].map((item) => (
            <li key={item}>
              <Link
                href={`#${item}`}
                onClick={toggleMenu}
                className="capitalize block px-3 py-2 rounded transition hover:text-primary hover:bg-gray-800"
              >
                {item}
              </Link>
            </li>
